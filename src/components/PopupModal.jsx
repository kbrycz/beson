    ...
    >
      ...
      >
        <div className="p-6 sm:p-10 md:p-14">
          <button onClick={handleClose} className="absolute top-5 right-5 w-10 h-10 flex items-center justify-center rounded-full bg-neutral-100 hover:bg-neutral-200 text-primary-950 transition-colors duration-200" aria-label="Close popup">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M6 18L18 6M6 6l12 12" /></svg>
          </button>

          <h2 className="text-2xl md:text-3xl lg:text-4xl font-extrabold text-primary-950 text-center mb-4 md:mb-8 leading-[1.15] md:leading-tight pr-8">
            {siteData.popup.headline}
          </h2>

          <ContactForm compact={true} buttonText="JOIN NOW" onSubmit={handleSubmit} />

          <p className="text-neutral-600 text-xs mt-6 text-center leading-relaxed">
            {siteData.popup.disclaimerText} For questions, reach out to{' '}
            <a href={`mailto:${siteData.contact.email}`} className="underline hover:no-underline transition-all text-primary-950 hover:text-accent-400">{siteData.contact.email}</a>. View our{' '}
            <a href={siteData.contact.privacyUrl} className="underline hover:no-underline transition-all text-primary-950 hover:text-accent-400">Privacy Policy</a>.
          </p>
        </div>
      </div>
    </div>
