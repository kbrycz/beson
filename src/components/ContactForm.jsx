...
function ContactForm({ compact = false, buttonText = 'JOIN US', onSubmit }) {
  ...
  const inputBaseClasses = 'w-full px-4 py-3 rounded-md border-2 border-neutral-200 bg-white text-primary-950 focus:outline-none focus:border-accent-400 transition-all text-lg'

  const inputInlineStyle = { backgroundColor: colors.white, borderColor: colors.neutral[200] }

  const handleFocus = (e) => {
    e.target.style.borderColor = colors.accent[400]
    e.target.style.boxShadow = `0 0 0 3px ${colors.accent[400]}30`
  }
  ...
  const placeholderStyle = `input::placeholder { font-weight: 600; color: rgba(0, 0, 0, 0.4); }`
    ...
    return (
      <form className="space-y-3" onSubmit={handleSubmit}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <button type="submit" className="w-full px-4 py-3 rounded-md bg-accent-400 hover:bg-accent-500 text-white font-semibold transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-accent-300 shadow-lg text-base uppercase tracking-wide">
            {buttonText}
          </button>
        </div>
      </form>
